...
def parse_sdf_file(path, shape_func=None):
    rdmol = next(iter(Chem.SDMolSupplier(path, removeHs=True)))
    return parse_rdkit_mol(rdmol, shape_func=shape_func)

def parse_rdkit_mol(rdmol, shape_func=None):
