...
def get_Hstat(mol):
    ...
    num_doners = Lipinski.NumHDonors(mol)
    return {'num_acceptors': num_acceptors, 'num_doner': num_doners}
...
if __name__ == '__main__':
    # mol = Chem.SDMolSupplier('/fs/ess/PCON0041/gruoxi/Drugs/Donepezil.sdf')[0] 
    # print('!!!!!!!!!!!!!!!!!')
    # print('Donepezil')
    # print(get_basic(mol))
    # print(get_chem(mol))
    ...
    # mol = Chem.SDMolSupplier('/fs/ess/PCON0041/gruoxi/Drugs/Galantamine.sdf')[0] 
    # print('!!!!!!!!!!!!!!!!!')
    # print('Galantamine')
    # print(get_basic(mol))
    # print(get_chem(mol))
    ...
    # mol = Chem.SDMolSupplier('/fs/ess/PCON0041/gruoxi/Drugs/Rivastigmine.sdf')[0] 
    # print('!!!!!!!!!!!!!!!!!')
    # print('Rivastigmine')
    # print(get_basic(mol))
    # print(get_chem(mol))
    # print(get_Hstat(mol))
    IPDiff_path = '/fs/scratch/PCON0041/gruoxi/IPDiff/test/results/eval_results/metrics_-1.pt'
    IPDiff_result = torch.load(IPDiff_path)

    ALIDiff_path = '/fs/scratch/PCON0041/gruoxi/ALIDiff/results/eval_results/metrics_-1.pt'
    ALIDiff_result = torch.load(ALIDiff_path)

    P2Diff_path = '/fs/scratch/PCON0041/gruoxi/2_27/test/results/test_sample_pocket_weighted_pool_dynamic_update_gvp_bond_type_late_bilevel_seperate_k32_k4_650k/eval_results/metrics_-1.pt'
    P2Diff_result = torch.load(P2Diff_path)

    TargetDiff_path = '/fs/scratch/PCON0041/gruoxi/TargetDiff/results/eval_results/metrics_-1.pt'
    TargetDiff_result = torch.load(TargetDiff_path)

    # for result in tqdm(P2Diff_result['all_results']):
    #     mol = result['mol']
    #     get_chem(mol)['logp']
    ...
    for complex in tqdm(split['train']):
        ...
        if rdmol != None:
            chem_results = get_chem(rdmol)
            logPs.append(chem_results['logp'])
            SAs.append(chem_results['sa'])
            QEDs.append(chem_results['qed'])

    with open("/fs/scratch/PCON0041/gruoxi/TargetDiff/results/QED_P2Diff.pkl", "wb") as f:
        pickle.dump(QEDs, f)
    with open("/fs/scratch/PCON0041/gruoxi/TargetDiff/results/SA_P2Diff.pkl", "wb") as f:
        pickle.dump(SAs, f)
    with open("/fs/scratch/PCON0041/gruoxi/TargetDiff/results/LogP_P2Diff.pkl", "wb") as f:
        pickle.dump(logPs, f)
    ...
    # RMSD = []
    # for result in tqdm(ALIDiff_result['all_results']):
    #     mol = result['mol']
    #     if get_rdkit_rmsd(mol)[1] != np.nan:
    #         RMSD.append(get_rdkit_rmsd(mol)[1])

    # with open("/fs/scratch/PCON0041/gruoxi/TargetDiff/results/RMSD_ALIDiff.pkl", "wb") as f:
    #     pickle.dump(RMSD, f)
